...
if [ -d "$PLATFORM_DIR/.git" ]; then
    ...
    echo "[OK] Z-ai-platform: CLONED"
    echo "  Branch: $branch"
    echo "  Last commit: $last_commit"
    echo "  Uncommitted changes: $dirty files"
else
    echo "[FAIL] Z-ai-platform: NOT CLONED"
    echo ""
...
symlink_count=$(find "$SANDBOX_SKILLS_DIR" -maxdepth 1 -type l 2>/dev/null | wc -l)
real_count=$(find "$SANDBOX_SKILLS_DIR" -maxdepth 1 -type d 2>/dev/null | wc -l)
real_count=$((real_count - 1))  # subtract the parent dir itself

if [ "$symlink_count" -eq 0 ]; then
    echo "[FAIL] No symlinks found -- bootstrap.sh has not been run in this session."
    echo ""
    echo "  -> Run:"
    echo "    bash $PLATFORM_DIR/bootstrap.sh"
else
    echo "[OK] $symlink_count custom skills linked from Z-ai-platform"
...
for skill in zai-skill-creator zai-skill-registry; do
    ...
    if [ -L "$target" ]; then
        ...
        if [ -f "$target/SKILL.md" ]; then
            echo "[OK] $skill -> $(basename $(dirname $real_path))/$(basename $real_path)"
        else
            echo "[FAIL] $skill -> BROKEN SYMLINK (target missing)"
        fi
    elif [ -d "$target" ]; then
        echo "[WARN] $skill -> real directory (sandbox-installed, not your version)"
    else
...
if [ -f "$SANDBOX_SKILLS_DIR/zai-skill-creator/SKILL.md" ]; then
    cool_count=$(grep -cE "Cool\?|how could you|plumbers|Good luck!" "$SANDBOX_SKILLS_DIR/zai-skill-creator/SKILL.md" 2>/dev/null || true)
    cool_count=${cool_count:-0}
    line_count=$(wc -l < "$SANDBOX_SKILLS_DIR/zai-skill-creator/SKILL.md")
    if [ "${cool_count:-0}" -eq 0 ]; then
        echo "[OK] Refactored version active ($line_count lines, no conversational tone)"
    else
        echo "[WARN] Original Anthropic version active ($cool_count conversational phrases found, $line_count lines)"
